...
The main checkout is `F:\GAIT`. This package contains the complete RC2 source, arranged for direct HEMTT builds. The gameplay files are unchanged from RC2.

## Install and deploy

Save `GAIT_v1_7_0_RC2_HEMTT.zip` to `M:\downloads`, then paste this complete block into PowerShell:

```powershell
& {
    $ErrorActionPreference = 'Stop'
    $Zip = 'M:\downloads\GAIT_v1_7_0_RC2_HEMTT.zip'
    $Extract = 'M:\downloads\GAIT_HEMTT'
...
It migrates `source\gait` to `addons\gait`, applies the complete RC2 source and HEMTT configuration, commits, and pushes `dev/gait-1.7.0-rc2`. Main is unchanged. A rejected push stops with an error; no force push or reset is performed. Close terminals/editors using the old folder if Windows reports it is in use.
...
See `README_TEST_BUILD.md` for the RC2 movement changes, tests and remaining Arma acceptance checks. This packaging update does not add an in-game test result.
