# GAIT 1.8.0-alpha7: build and deployment

Save `GAIT_v1_8_0_ALPHA7_HEMTT.zip` to `M:\downloads`. Close Arma before rebuilding. HEMTT and Git must be on PATH. This test build requires Arma 3 2.18 or later.
...
& {
    ...
    $Extract = 'M:\downloads\GAIT_Foundation_1_8_0_ALPHA7'
    Expand-Archive -LiteralPath 'M:\downloads\GAIT_v1_8_0_ALPHA7_HEMTT.zip' `
...
The PBO's virtual prefix remains `gait`, preserving asset and function paths. The source ZIP contains existing PAA/OGG assets, tests and HEMTT configuration; generated build output is excluded. HEMTT compiles sixteen addon SQF files and builds two PBOs. The optional gait_heartbeat.pbo halves ACE Medical Feedback heartbeat gain and skips loading if that ACE component is absent. Python is unnecessary for a normal build because the generated graph is already included.

Load CBA_A3, ACE3 and the new local mod. Unload older GAIT copies and start a fresh mission with GAIT and ACE AF enabled. The runtime/HUD version is **1.8.0-alpha7**. Follow `README_TEST_BUILD.md` for heartbeat, ADS sway, fatigue vignette and movement checks.
