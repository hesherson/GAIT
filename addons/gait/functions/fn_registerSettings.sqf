...
["GAIT_ss_slopeLocomotionEnabled", "Enable dedicated sprint animation family", "Uses a dedicated native direction map throughout standing sprint, keeping the same running family when terrain gets steeper. Walking selections map to running states, with native strafe transitions. Experimental until tested in Arma; no maximum slope angle.", _categorySlope, true] call _addCheckbox;
