...
["GAIT_ss_lightSpeedBonus", "Light kit speed multiplier", "Overall speed multiplier for light kits. Default: 1.08.", _categoryWeight, 0.50, 1.50, 1.08, 3] call _addSlider;
["GAIT_ss_mediumSpeedBonus", "Medium kit speed multiplier", "Overall speed multiplier for medium kits. Default: 1.05.", _categoryWeight, 0.50, 1.50, 1.05, 3] call _addSlider;
["GAIT_ss_moderateSpeedBonus", "Moderate kit speed multiplier", "Overall speed multiplier for moderate kits. Default: 1.025.", _categoryWeight, 0.50, 1.50, 1.025, 3] call _addSlider;
["GAIT_ss_heavySpeedBonus", "Heavy kit speed multiplier", "Overall speed multiplier for heavy kits. Default: 1.00.", _categoryWeight, 0.50, 1.50, 1.00, 3] call _addSlider;
...
["GAIT_ss_uphillMaxDegrees", "Uphill max slowdown angle", "Incline angle where uphill slowdown reaches its maximum. A higher value makes the slowdown ramp in more gradually. Default: 35 degrees.", _categorySlope, 5.00, 80.00, 35.00, 1] call _addSlider;
["GAIT_ss_uphillMaxPenalty", "Uphill max speed penalty", "Maximum configured sprint-speed reduction on steep uphill terrain before the minimum multiplier is applied. 0.40 means up to 40% slower. Default: 0.40.", _categorySlope, 0.00, 0.95, 0.40, 2] call _addSlider;
["GAIT_ss_uphillMinSprintMultiplier", "Uphill minimum sprint speed", "Lowest allowed uphill sprint-speed multiplier. 0.60 means steep uphill sprinting bottoms out at 60% of the current GAIT sprint speed instead of falling into Arma's walk behavior. Default: 0.60.", _categorySlope, 0.10, 1.00, 0.60, 2] call _addSlider;
...
["GAIT_ss_hillWalkSlowdownMaxDegrees", "Hill walk slowdown max angle", "Slope angle where walking slowdown reaches full strength. Default: 40 degrees.", _categorySlope, 5.00, 80.00, 40.00, 1] call _addSlider;
["GAIT_ss_hillWalkUphillMaxPenalty", "Uphill walk max penalty", "Maximum normal movement speed loss while walking uphill before sprinting. Default: 0.32.", _categorySlope, 0.00, 0.75, 0.32, 2] call _addSlider;
...
["GAIT_ss_debugHudEnabled", "Enable debug HUD", "Shows a live corner readout for speed, slope, momentum, coast/brace state, rescue pulse age, stamina state, and compatibility scan hits. Default: disabled.", _categoryQoL, false] call _addCheckbox;
...
["GAIT_ss_maxSprintSlopeDegrees", "Maximum ACE slope override", "Highest terrain steepness where GAIT removes ACE fatigue movement locks. Above this, ACE keeps its restrictions. Native Arma terrain limits still apply. Requires the ACE bridge and walk-lock override. Two-degree re-entry band. Default: 38 degrees.", _categorySlope, 20, 50, 38, 0] call _addSlider;
