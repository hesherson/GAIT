...
| Slope stop brace (`GAIT_ss_slopeStopBraceEnabled`) | true | on / off | If enabled, stopping and restarting sprint on an incline makes the initial brace step more pronounced based on slope angle. Default: enabled. |
| Slope brace starts (`GAIT_ss_slopeStopBraceStartDegrees`) | 15 | 0–60 | Incline/decline angle where slope-aware brace begins. Default: 15 degrees. |
| Slope brace max angle (`GAIT_ss_slopeStopBraceMaxDegrees`) | 35 | 5–80 | Slope angle where the extra brace reaches full strength. Default: 35 degrees. |
| Slope brace extra duration (`GAIT_ss_slopeStopBraceExtraDuration`) | 0.18 | 0–2 | Extra brace duration added on steep inclines/declines. Default: 0.18 sec. |
| Slope brace extra dip (`GAIT_ss_slopeStopBraceExtraDip`) | 0.28 | 0–0.9 | Additional speed dip applied to the brace step on steep slopes. Default: 0.28. |
...
| Enable downhill speed bonus (`GAIT_ss_downhillBoostEnabled`) | true | on / off | Builds a load-dependent bonus during actual downhill sprint travel. Gain tapers again on extreme descents. Default: enabled. |
...
| Downhill max boost angle (`GAIT_ss_downhillBoostMaxDegrees`) | 18 | 5–60 | Decline angle where the downhill speed boost reaches its maximum configured value. Default: 18 degrees. |
| Downhill base speed boost (`GAIT_ss_downhillMaxBoost`) | 0.06 | 0–0.35 | Base unloaded downhill bonus at full momentum, added to the sustained bonus below. Both scale down with kit weight and extreme descent angle. Default: 0.06. |
| Sustained downhill bonus (`GAIT_ss_downhillSustainedExtraBoost`) | 0.12 | 0–0.25 | Additional unloaded downhill bonus built by actual sprint travel. Combined bonus is capped at 35%, reduced by kit weight, and tapered above 35 degrees. Zero removes this extra bonus. Default: 0.12. |
| Downhill momentum build time (`GAIT_ss_downhillMomentumBuildSeconds`) | 2.5 | 0.5–8 | Seconds of actual sprint travel to build 95% of downhill momentum. Releasing sprint allows decay while moving; releasing forward movement clears it. Default: 2.5 seconds. |
